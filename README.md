...
该数据为从新浪微博不实信息举报平台抓取的中文谣言数据，分为两个部分，其中CED_Dataset为包含转发/评论信息的中文谣言数据集，而当前目录下JSON文件中的数据集不包含转发/评论信息。

## 数据集介绍（不含转发/评论信息）

该数据集（./）共包含从2009年9月4日至2017年6月12日的31669条谣言。文件中，每一行为一条json格式的谣言数据，字段释义如下：
